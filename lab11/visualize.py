...
class Scene(Window):
    def __init__(self, M, width, height, threshold=6.5, thresholds=(0.5,19.5,1.0)):
        ...
        #self.points = part1.march(M, threshold) # part 1
    ...
    def on_draw(self, dt=0):
        ...
        #print('-'*30)
        for threshold, points in self.points: 
            for pcase,cell in points:
                #print(f'case {pcase}:',cell)
                #print()
                for x,y in cell:
                    x,y = self.matrix2world(x,y)
                    glVertex3f(x,threshold*scale,y)
        glEnd()
        
        for threshold, points in self.points:
            for pcase,cell in points:
                c = 1.0#(0.3 + threshold) / 22.5
                glColor4f(c, c, c, 1.0)
                glBegin(GL_LINES)
                for x,y in cell:
                    x,y = self.matrix2world(x,y)
